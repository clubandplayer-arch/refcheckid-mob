...
describe("regression: referee report validation", () => {
  it("blocks goals inserted outside chronological order", () => {
    ...
    expect(errors.includes("Gol: eventi non in ordine cronologico.")).toBe(true);
  });
  ...
  it("blocks report submission when goal events do not match the final score", () => {
    ...
    expect(errors.includes("Gol: gol Casa inseriti 2/1.")).toBe(true);
    expect(errors.includes("Gol: numero eventi superiore al risultato finale.")).toBe(true);
  });
  ...
  it("requires goal events to match both home and away final score totals", () => {
    ...
    expect(errors.includes("Gol: gol Ospite inseriti 0/1.")).toBe(true);
  });
  ...
  it("accepts roster-backed player names and shirt numbers", () => {
    ...
    expect(errors.includes("Gol: tesserato non coerente con squadra e maglia.")).toBe(false);
    expect(errors.includes("Gol: tesserato squalificato non selezionabile.")).toBe(false);
  });
  ...
  it("blocks shirt numbers that are not in the selected team sheet", () => {
    ...
    expect(errors.includes("Gol: tesserato non presente nella distinta della squadra.")).toBe(true);
  });
  ...
  it("blocks a reserve scoring before being substituted in", () => {
    ...
    expect(errors.includes("Gol: riserva non entrata in campo al minuto 31.")).toBe(true);
  });
  ...
  it("blocks events before a substitute has entered", () => {
    ...
    expect(errors.includes("Ammonizioni: tesserato non ancora entrato al minuto 50.")).toBe(true);
  });
  ...
  it("blocks duplicate substitution players across rows", () => {
    ...
    expect(errors.includes("Sostituzioni: tesserato già usato alla riga 2.")).toBe(true);
  });
  ...
  it("blocks invalid minutes and substitution with same player", () => {
    ...
    expect(errors.includes("Ammonizioni: minuto non valido alla riga 1.")).toBe(true);
    expect(errors.includes("Sostituzioni: entrante e uscente devono essere diversi.")).toBe(true);
  });
});
